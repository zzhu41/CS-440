...
double alpha_beta(vector<vector<int>> board, int current_depth,int max_depth, int start_color, int current_color, int remaining_black, int remaining_white, int strategy,double a,double b){
    ...
    if(current_depth == max_depth){
    	if(start_color == 1){ // if own is black
    		...
    		else if(strategy == 2){ // offensive heuristic 2
    			// double cur_heuristic = (7*remaining_white) + 12*(30-remaining_black) + ((double) rand() / (RAND_MAX));
    			double cur_heuristic = 12*(30-remaining_black) + cal_heuristic_white(board) + ((double) rand() / (RAND_MAX));
    			return cur_heuristic;
    		}
    		else if(strategy == 3){ // defensive heuristic 2
    			double cur_heuristic = 5*(remaining_white) + 3*(30 - remaining_black) + cal_heuristic_white(board) + ((double) rand() / (RAND_MAX));
    			return cur_heuristic;
    		}
    	}
    }
	...
	for(int l = 0; l < 10; l++){
		...
		while(!win){
			...
			if(start%2 == 1){//white's turn
				...
				alpha_beta(board,0,4,2,2,black_count,white_count,2,INT_MIN,INT_MAX);
