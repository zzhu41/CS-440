...
for i in range(10):#For each number class
	...
	for j in range(len(data_dict[i])):
		...
		for x_idx in range(len(num_dict)/m):
			for y_idx in range(len(num_dict[x_idx])/n):
				#obj = (x_idx, y_idx, data_dict[i][j][x_idx][y_idx])
				wenwen = ''
				c = 0
				for x in range(m):
					for y in range(n):
						#wenwen += data_dict[i][j][x_idx*m+x][y_idx*n+y]
						if data_dict[i][j][x_idx*m+x][y_idx*n+y] == '#' :
							wenwen += '#'
						if data_dict[i][j][x_idx*m+x][y_idx*n+y] == '+' :
							wenwen += '#'
						if data_dict[i][j][x_idx*m+x][y_idx*n+y] == ' ' :
